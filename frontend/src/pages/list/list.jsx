...
export const List = () => {
  ...
  const updateMovies = async () => {
    try {
      const { payload } = await getList({ idList: id });
          ...
          <section className="mt-9">
            ...
            <div className="mt-4 grid grid-cols-2  sm:grid-cols-4 gap-x-5 gap-y-5">
              {movies && movies.map(({ movie }) => {
                const { id, title = '', description = '', thumbnail ='' } = movie;
                return <Film key={id} title={title} imgUrl={thumbnail} overview={description}></Film>;
              })}
            </div>
          </section>
        </main>
